...
//		graphics.setColor(Color.BLACK);
		...
		graphics.drawText((String) child.getField("id"), secondRowPosition,
				yStartForText,
				(DrawStyle.LEFT | DrawStyle.ELLIPSIS | DrawStyle.TOP), width);

		yStartForText = y + (this.getFont()).getHeight() + 1;
		graphics.setFont(rowFont);
		graphics.drawText((String) child.getField("last_known_location"),
				secondRowPosition, yStartForText, (DrawStyle.LEFT
						| DrawStyle.ELLIPSIS | DrawStyle.TOP), width);

		yStartForText = yStartForText + (this.getFont()).getHeight() + 1;
		graphics.setFont(rowFont);
		graphics.drawText((String) child.getField("date_of_separation"),
				secondRowPosition, yStartForText, (DrawStyle.LEFT
						| DrawStyle.ELLIPSIS | DrawStyle.TOP), width);
